...
typedef struct{
	uint8_t I2C_MODE;				 // master or slave 
	uint8_t I2C_BUS_FREQUENCY;				 // to select the division factor for the bit rate generator
	I2C_PRESCALE_t I2C_PRESCALER;	 // to specify state of the I2C Prescaler Bits
	uint8_t I2C_INT;					 // to enable interrupt
	uint8_t I2C_SLAVE_ADDRESS;			 // to determine the slave address of the controller in case if it becomes slave in the network
	uint8_t I2C_GEN_CALL;			 // to enable/disable the recognition of a General Call given over the Two-wire Serial Bus
 ...
 * 
 */
void MCAL_I2C_Slave_TX(uint8_t TxData);
...
void I2C_ACKConfig(I2C_Config_t *I2C_Config, Functional_State State);
